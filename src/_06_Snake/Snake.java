...
public class Snake {
	...
	public void update() {
		...
		case UP:
			newL = new Location(head.getLocation().x, head.getLocation().y - 1);
		...
		case DOWN:
			...
			break;
		case RIGHT:
			newL = new Location(head.getLocation().x + 1, head.getLocation().y);
			// head.setLocation(newL);
			break;
		case LEFT:
			newL = new Location(head.getLocation().x - 1, head.getLocation().y);
		...
		for (int i = snake.size() - 1; i >= 0; i--) {
			if (i > 0) {
				...
				snake.get(i).setLocation(head.getLocation());
			}
		}
	...
	public void reset(Location loc) {
		...
		head.setLocation(new Location(15 / 2, 6));
	...
	public boolean isOutOfBounds() {
		...
		// of the window
		// and false otherwise
		if (head.getLocation().x < -1 || head.getLocation().x > 15 || head.getLocation().y < -1
